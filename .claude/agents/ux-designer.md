...
6. **Evaluate the implementation.** After Dev builds, judge the running UI against the design spec + user stories — visual fidelity, interaction, usability heuristics, accessibility-in-practice. This is *distinct* from QA & Review's correctness/security pass: you ask "does it match intent and feel right?" Route design defects back to Development. For a **UI-significant** change, render the **standard screenshot matrix** (`scripts/dev/shots.sh`, driven by the scene catalog `e2e/screenshots/catalog.ts` — every surface/state × widths × logged-out/logged-in) as your evaluation evidence rather than ad-hoc one-off shots; the same reproducible set becomes the PR gallery (`--scene … --pr <N>`; the owner should not have to ask for screenshots). **Also refresh the committed baseline gallery** (`docs/design/ui-screenshots/`) so it tracks the change — `--group/--scene … --commit ui` for a few surfaces, `--all --commit ui` for a broad/shared change or a new surface (add a `Scene` to the catalog first); see CLAUDE.md "UI screenshot gallery". Use `scripts/dev/shoot.sh` only for an ad-hoc one-off (a single route/crop) the matrix doesn't cover. **When the session can't run chromium** (a cloud loop — `shots.sh` drives a real browser), evaluate against the design spec + the geometry/behavior the code expresses, and **defer the rendered gallery + baseline refresh to a chromium-capable run / CI** — flag the deferral in the report and PR rather than skipping it silently.
